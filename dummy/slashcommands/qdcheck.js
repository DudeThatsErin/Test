...
module.exports = {
    name: 'qdcheck',
    description: 'Quick check of developer service status.',
    execute(interaction) {

    // Function for quick developer service check
      console.log("Quick checking developer service status from local file...");
      try {
        const data = fs.readFile("dev_service_status.json", "utf8");
        ...
        let embed = new EmbedBuilder()
          ...
          .setTitle("Quick check report of developer service status:");
        ...
        if (!embed.data.fields || embed.data.fields.length === 0) {
          embed.setDescription("**🟢 All developer services are up.**");
        }

        interaction.reply({ embeds: [embed] });
      } catch (error) {
        console.error("Error quick checking developer service status:", error);
        interaction.reply({
          embeds: [
            new EmbedBuilder()
              .setColor(0xf8312f)
              .setTitle("Quick check report of developer service status:")
              .setDescription("Error fetching developer service status."),
          ],
        });
      }
